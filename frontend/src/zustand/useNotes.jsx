...
export const useNotes = create((set, get) => ({
  ...
  getNotes: async () => {
    ...
    try {
      const response = await axios.get(
        "https://notesapp-7sbx.onrender.com/get-all-notes",
        {
          headers: {
  ...
  addToNotes: async (title, content, tags) => {
    ...
    try {
      const response = await axios.post(
        "https://notesapp-7sbx.onrender.com/add-note",
        { title, content, tags },
        {
          headers: {
            Authorization: token, // Token already includes 'Bearer '
  ...
  updateNote: async (noteId, updatedData) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://notesapp-7sbx.onrender.com/edit-note/${noteId}`, // Changed to http
        updatedData,
        {
          headers: {
            Authorization: token, // Token already includes 'Bearer '
  ...
  deleteNote: async (noteId) => {
    ...
    try {
      const response = await axios.delete(
        `https://notesapp-7sbx.onrender.com/delete-note/${noteId}`, // Changed to http
        {
          headers: {
            Authorization: token, // Token already includes 'Bearer '
          },
  ...
  togglePinNote: async (noteId, isPinned) => {
    ...
    try {
      const response = await axios.put(
        `https://notesapp-7sbx.onrender.com/update-note-isPinned/${noteId}`,
        { isPinned: !isPinned },
        {
          headers: {
            Authorization: token,
